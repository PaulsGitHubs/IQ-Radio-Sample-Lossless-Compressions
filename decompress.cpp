...
int main() {
    ...
    if(!input_real.is_open()) {
        std::cerr << "Failed to open dwt_real.bin" << std::endl;
        return 1;
    }
    ...
    if(!input_imag.is_open()) {
        std::cerr << "Failed to open dwt_imag.bin" << std::endl;
        return 1;
    }
    ...
    wave_object obj_real = wave_init("db4"); // Initialize wavelet
    if(obj_real == NULL) {
        std::cerr << "Failed to initialize wavelet for real part" << std::endl;
        return 1;
    }

    wt_object wt_real = wt_init(obj_real, "idwt", N, 4); // Initialize inverse discrete wavelet transform object with 4 levels
    ...
    wave_object obj_imag = wave_init("db4"); // Initialize wavelet
    if(obj_imag == NULL) {
        std::cerr << "Failed to initialize wavelet for imaginary part" << std::endl;
        return 1;
    }

    wt_object wt_imag = wt_init(obj_imag, "idwt", N, 4); // Initialize inverse discrete wavelet transform object with 4 levels
